...
--
-- Hoszt: localhost
-- Létrehozás ideje: 2015. Aug 14. 07:36
...
,`default_detail_view` varchar(100)
...
`id_content` int(11) unsigned NOT NULL,
  ...
  `detail_view` varchar(100) DEFAULT NULL,
...
INSERT INTO `contents` (`id_content`, `code`, `id_type`, `ordering`, `id_author`, `author`, `created`, `id_updater`, `updater`, `updated`, `publish_on`, `publish_off`, `logical_date`, `indexed`, `has_url`, `flag`, `priority`, `default_language`, `site_root`, `extends_data`, `list_view`, `single_view`, `detail_view`, `deleted`) VALUES
(1, 'homepage', 1, 0, 1, 'contact@liszkaiadam.hu', '2015-08-12 15:09:28', NULL, NULL, NULL, NULL, NULL, NULL, 0, 1, NULL, 5, 1, 1, NULL, NULL, NULL, NULL, NULL);
...
(1, 1, 1, 'en_US', 'Welcome', NULL, NULL, NULL, 'home', 'welcome', NULL, NULL, NULL, NULL, NULL),
(2, 1, 2, 'hu_HU', 'Üdvözlet', NULL, NULL, NULL, 'kezodlap', 'nyitolap', NULL, NULL, NULL, NULL, NULL);
...
`id_type` int(11) unsigned NOT NULL,
  ...
  `default_detail_view` varchar(100) DEFAULT NULL,
...
INSERT INTO `contents_types` (`id_type`, `code`, `ordering`, `default_list_view`, `default_single_view`, `default_detail_view`, `default`) VALUES
...
INSERT INTO `users` (`id_user`, `login`, `encryption`, `hash`, `password`, `name`, `email`, `forname`, `lastname`, `gender`, `birthday`, `activated`, `registered`, `id_role`, `id_language`, `language`, `last_login`, `last_ip`, `login_try`, `extend_data`, `deactivated`, `deleted`) VALUES
(1, 'adam.liszkai', 'text', NULL, 'Ind3pender', 'Adam Liszkai', 'contact@liszkaiadam.hu', 'Adam', 'Listkai', NULL, NULL, '2015-08-12 17:07:10', '2015-08-12 14:41:58', 1, 2, 'hu_HU', NULL, NULL, 0, NULL, NULL, NULL);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `content` AS select `cnt`.`id_content` AS `id_content`,`cnt`.`code` AS `name`,`cnt`.`id_type` AS `id_type`,`ctp`.`code` AS `type`,`cnt`.`ordering` AS `ordering`,`cnt`.`publish_on` AS `publish_on`,`cnt`.`publish_off` AS `publish_off`,`cnt`.`logical_date` AS `logical_date`,`cnt`.`extends_data` AS `extends_data`,`ctp`.`default_list_view` AS `default_list_view`,`ctp`.`default_single_view` AS `default_single_view`,`ctp`.`default_detail_view` AS `default_detail_view`,`cnt`.`list_view` AS `list_view`,`cnt`.`single_view` AS `single_view`,`cnt`.`detail_view` AS `detail_view`,`lng`.`id_language` AS `id_language`,`lng`.`code` AS `language`,`ctr`.`title` AS `title`,`ctr`.`subtitle` AS `subtitle`,`ctr`.`preview` AS `preview`,`ctr`.`window` AS `window`,`ctr`.`short_url` AS `short_url`,`ctr`.`long_url` AS `long_url`,`ctr`.`content` AS `content`,`ctr`.`keywords` AS `keywords`,`ctr`.`description` AS `description`,`ctr`.`extends_data` AS `extends_data_lang`,`cnt`.`indexed` AS `indexed`,`cnt`.`has_url` AS `has_url`,`cnt`.`flag` AS `flag`,`cnt`.`priority` AS `priority`,`ath`.`id_user` AS `id_author`,`cnt`.`created` AS `created`,`ath`.`login` AS `author_login`,`ath`.`name` AS `author_name`,`ath`.`forname` AS `author_forname`,`ath`.`lastname` AS `author_lastname`,`ath`.`email` AS `author_email`,`upd`.`id_user` AS `id_updater`,`cnt`.`updated` AS `updated`,`upd`.`login` AS `updater_login`,`upd`.`name` AS `updater_name`,`upd`.`forname` AS `updater_forname`,`upd`.`lastname` AS `updater_lastname`,`upd`.`email` AS `updater_email`,`cnt`.`default_language` AS `default_language`,`cnt`.`site_root` AS `homepage` from (((((`contents_translations` `ctr` left join `languages` `lng` on((`ctr`.`id_language` = `lng`.`id_language`))) left join `contents` `cnt` on((`ctr`.`id_content` = `cnt`.`id_content`))) left join `contents_types` `ctp` on((`cnt`.`id_type` = `ctp`.`id_type`))) left join `users` `ath` on((`cnt`.`id_author` = `ath`.`id_user`))) left join `users` `upd` on((`cnt`.`id_updater` = `upd`.`id_user`))) where isnull(`cnt`.`deleted`);
...
ALTER TABLE `contents`
 ADD PRIMARY KEY (`id_content`), ADD KEY `content_type` (`id_type`), ADD KEY `id_content` (`id_content`,`code`), ADD KEY `content_author` (`id_author`,`author`), ADD KEY `content_updater` (`id_updater`,`updater`);
...
ALTER TABLE `languages`
 ADD PRIMARY KEY (`id_language`,`code`,`url`);
...
--
ALTER TABLE `navigations`
MODIFY `id_navigation` int(11) unsigned NOT NULL AUTO_INCREMENT;
--
