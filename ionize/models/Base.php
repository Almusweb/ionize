	...
	public function get($table=NULL)
	{
		...
		if($query->num_rows() > 0)
		{
			...
			if($query->num_rows() == 1)
			{
				...
				foreach($row as $name => $value) if(property_exists($this, $name)) $this->$name = $value;	
				$this->id = $row->{$this->id_field};
			}
			else foreach($row as $name => $value) if(property_exists($this, $name)) $this->$name = NULL;
		}		
